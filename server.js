...
// ── The Odds API 金鑰（從環境變數讀取）──
const THEODDS_KEY = (process.env.THEODDS_KEY || '').trim();
...
const server = http.createServer(async (req, res) => {
  ...
  // ── Proxy: The Odds API 盤口（金鑰由伺服器附加）──
  // 前端呼叫 /api/odds/sports/soccer_world_cup/odds?...
  if (pathname.startsWith('/api/odds/')) {
    if (!THEODDS_KEY) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: '伺服器未設定 THEODDS_KEY 環境變數' }));
      return;
    }
    const endpoint = pathname.replace('/api/odds', '');
    const q = parsed.search || '';
    const sep = q ? '&' : '?';
    try {
      const result = await httpsRequest({
        hostname: 'api.the-odds-api.com',
        path: '/v4' + endpoint + q + sep + 'apiKey=' + encodeURIComponent(THEODDS_KEY),
        method: 'GET'
      });
      res.writeHead(result.status, { 'Content-Type': 'application/json' });
      res.end(result.body);
...
server.listen(PORT, () => {
  ...
  console.log(`🎲 The Odds API 金鑰：${THEODDS_KEY ? '已設定 ✓' : '未設定 ✗（請設環境變數 THEODDS_KEY）'}`);
  console.log(`\n按 Ctrl+C 停止伺服器\n`);
});
